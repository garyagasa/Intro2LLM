...
class PromptRawDataset(object):
    ...
    def __init__(self, output_path, seed, local_rank, dataset_name):
        ...
        if 'MyDataset' not in dataset_name:
...
def my_load(filepath):
    with open(filepath, 'r') as fp:
        data = json.load(fp)
    # prompts = [item['questions'] for item in data['train']]
    # answers = [item['answers'] for item in data['train']]
    train_dataset = Dataset.from_dict({"prompt": [item['questions'] for item in data['train']], "chosen": [item['answers'] for item in data['train']]})
    eval_dataset = Dataset.from_dict({"prompt": [item['questions'] for item in data['eval']], "chosen": [item['answers'] for item in data['eval']]})
    ...
    # dataset.rename_column("questions", "prompt").rename_column("answers", "chosen")
    return dataset
...
class MyDataset(PromptRawDataset):
    def __init__(self, output_path, seed, local_rank, dataset_name):
        ...
        self.dataset_name_clean = "MyDataset"
