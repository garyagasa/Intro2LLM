...
def generate_hashes(args):
    if not os.path.exists(f"{args.output_dir}/minhash_nfc"):
        os.makedirs(f"{args.output_dir}/minhash_nfc")
        ...
        for i, chunks in enumerate(
            tqdm(
                pool.imap(
                    to_minhash,
                    zip(
                        chunked(documents, args.n_docs // cpu_num),
                        repeat(args.output_dir),
                        repeat(args.w),
                        repeat(args.dataset_name),
                        repeat(args.n_docs // cpu_num),
                    ),
                ),
                total=cpu_num,
            )
...
if __name__ == "__main__":
    ...
    args.index_end = None if args.index_end == 0 else args.index_end
